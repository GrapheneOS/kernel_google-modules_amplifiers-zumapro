...
int tas256x_update_default_params(struct tas256x_priv *p_tas256x, int ch)
{
	...
	if (p_tas256x->devs[ch-1]->device_id == DEVICE_TAS2562) {
		...
		p_tas256x->devs[ch-1]->icn_thr = 45;
		p_tas256x->devs[ch-1]->icn_hyst = 0;
		p_tas256x->devs[ch-1]->noise_gate = 0;
	...
	if ((value >= 0) && (value < 85)) {
		if (ch == channel_both) {
			for (i = 0; i < p_tas256x->mn_channels; i++) {
				n_result = p_tas256x->bulk_write(p_tas256x, i+1,
			...
			n_result = p_tas256x->bulk_write(p_tas256x, ch,
				...
				(char *)&(icn_thr[value][0]), sizeof(int));
			if (n_result == 0)
				p_tas256x->devs[ch-1]->icn_thr = value;
		}
	}
	...
	if ((value >= 0) && (value < 20)) {
		...
		if (ch == channel_both) {
			...
			n_result = p_tas256x->bulk_write(p_tas256x, ch,
				...
				data, sizeof(int));
			if (n_result == 0)
				p_tas256x->devs[ch-1]->icn_hyst = value;
		}
	}
...
int tas256x_enable_noise_gate(struct tas256x_priv *p_tas256x, int enable, int ch)
{
	int n_result = 0;

	n_result = p_tas256x->update_bits(p_tas256x,
		ch, TAS256X_ICN_SW_REG, TAS256X_ICN_NG_MASK,
		(enable << TAS256X_ICN_NG_SHIFT));

	if (n_result == 0)
		p_tas256x->devs[ch-1]->noise_gate = enable;
